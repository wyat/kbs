...
void free_brc_cache(char *userid){
	...
	if( strcmp( userid ,"guest") ){
        if (session->brc_cache_entry)
            munmap(session->brc_cache_entry,BRC_CACHE_NUM*sizeof(struct _session->brc_cache_entry));
    }
}
#endif
...
void init_brc_cache(const char* userid,bool replace) {
	...
	sprintf(dirfile,BBSHOME "/%s",temp);
        if(stat(dirfile, &st)<0) {
            char brc[BRC_CACHE_NUM*sizeof(struct _session->brc_cache_entry)];
            brcfdr = open(dirfile, O_RDWR|O_CREAT, 0600);
            memset(brc, 0, BRC_CACHE_NUM*sizeof(struct _session->brc_cache_entry));
            write(brcfdr, brc, BRC_CACHE_NUM*sizeof(struct _session->brc_cache_entry));
            close(brcfdr);
        }
        brcfdr = open(dirfile, O_RDWR, 0600);
	if (brcfdr==-1) bbslog("3error","can't open %s errno %d",dirfile,errno);
        session->brc_cache_entry = mmap(NULL, BRC_CACHE_NUM*sizeof(struct _session->brc_cache_entry), PROT_READ|PROT_WRITE, MAP_SHARED, brcfdr, 0);
...
{                               /* 删除 禁止POST用户 */
    ...
    if (HAS_PERM(user, PERM_SYSOP) || HAS_PERM(user, PERM_OBOARDS)) {
        ...
        fprintf(fn1, "来  源: %s \n", SHOW_USERIP(NULL, getSession()->fromhost));
        fprintf(fn1, "\n");
