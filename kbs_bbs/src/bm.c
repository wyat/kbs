        ...
        if (*ans == 'A' || *ans == 'a') {
            ...
            {   sprintf(filebuf,"%-12s","Î¥·´Ä³ÌõÕ¾¹æ");
			   ...
			   fprintf(fn,"À´  Ô´: bbs.zixia.net\n") ;
                        ...
                        fprintf(fn,"\n");
			fprintf(fn,"                            ´ó»°Î÷ÓÎÕ¾Îñ×éÖµ°àÕ¾Îñ£º\x1b[4m%s\x1b[0m\n",currentuser->userid);
                        fprintf(fn,"                              %s\n",ctime(&now));
                        strcpy(currentuser->userid,"SYSOP");
			strcpy(currentuser->username,"¸«Í·°ï°ïÖ÷Áî");
			strcpy(currentuser->realname,"¸«Í·°ï°ïÖ÷");
                    }
                    else
                    {		my_flag=1;
                        fprintf(fn,"¼ÄÐÅÈË: %s \n",currentuser->userid) ;
