...
int     real;
{
    ...
    } else if( u->lastlogin - u->firstlogin < 3 * 1800 ) {
...
int     real, unum;
{
    ...
    case '1':
        ...
        if( real ) {
            ...
            /*            if( atoi( buf ) > 0 ) newinfo.numposts = atoi( buf );*/
             ...
             sprintf( genbuf, "½«×¢²áÈÕÆÚÌáÇ°30·ÖÖÓ [Y/N]");
             getdata( i++, 0, genbuf, buf, 16, DOECHO, NULL, YEA);
             if(buf[0]=='y'||buf[0]=='Y') newinfo.firstlogin-=3*1800;
    ...
    case '2':
        ...
        {
            prints("\n\n´íÎó!ÏµÍ³½ûÖ¹ĞŞ¸ÄSYSOPµÄÃÜÂë,×ÏÏ¼ÕıÔÚÀ´µÄÂ·ÉÏ :)");
            pressreturn();
            clear();
            return 0;
        }
...
x_fillform()
{
    ...
    if ( (time(0)-currentuser->firstlogin) < 1800 /*3*86400*/) 
    {
        prints( "ÄúÊ×´ÎµÇÈë±¾Õ¾Î´Âú30·ÖÖÓ..." );
        prints( "ÇëÏÈËÄ´¦ÊìÏ¤Ò»ÏÂ£¬ÔÚÂú30·ÖÖÓÒÔºóÔÙÌîĞ´×¢²áµ¥¡£");
        pressreturn();
        return;
    }
    ...
    while( 1 ) {
        ...
        getfield( 14, "Äê.ÔÂ.ÈÕ(¹«Ôª)(±ØĞëÌîĞ´)",     "³öÉúÄêÔÂ", birth, STRLEN );
