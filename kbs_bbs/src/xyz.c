...
x_level()
{
    ...
    if (newlevel == lookupuser->userlevel)
        ...
        if ((lookupuser->userlevel & PERM_BOARDS ) && flag==0 )
            mail_file("etc/forbm",lookupuser->userid,"ÐÂÈÎ¶þµ±¼ÒµÄ±Ø¶Á");
        /* Bigman 2000.1.5 ÐÞ¸ÄÈ¨ÏÞ×Ô¶¯·¢ÐÅ */
        if ((lookupuser->userlevel & PERM_CLOAK ) && flag1==0 )
            mail_file("etc/forcloak",lookupuser->userid,"°ïÖ÷ÊÚÓèÄúÒþÉíÈ¨ÏÞ");
        if ((lookupuser->userlevel & PERM_XEMPT ) && flag2==0 )
            mail_file("etc/forlongid",lookupuser->userid,"°ïÖ÷ÊÚÓèÄú³¤ÆÚÕÊºÅÈ¨ÏÞ");
    }
    pressreturn() ;
    clear() ;
    return 0 ;
