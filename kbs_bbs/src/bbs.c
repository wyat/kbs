...
int noreply_post(struct _select_def* conf,struct fileheader *fileinfo,void* extraarg)
{
	...
	if(ans[0]=='2') return zhiding_post(conf,fileinfo,extraarg);
...
//new_i_read TODO!!!  zsend_post(0, fptr, arg->direct);
        ...
        case 'y':
        case 'r':
//new_i_read TODO!!!  do_reply(conf,fptr);     /*»ØÐÅ */
        case ' ':
        case '\n':
        case KEY_DOWN:
            *continue_flag = 1;
...
int range_flag(struct _select_def* conf,struct fileheader *fileinfo,void* extraarg)
{
    ...
    if(i>=1&&i<=total) {
        f.filename[0]=0;
        change_post_flag(currBM, currentuser, arg->mode, currboard->filename, i, &f, arg->direct, fflag, 0);
    }
    prints("\nÍê³É±ê¼Ç\n");
    pressreturn();
    return DIRCHANGED;
