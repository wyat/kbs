...
unsigned lvl;
{
    ...
    /* Bigman: Ôö¼ÓÖÐÎÄ²éÑ¯ÏÔÊ¾ 2000.8.10 */
     // buf[10], buf ×î¶à 4 ¸öºº×Ö + 1 byte £¨\0½áÎ²£©
      ...
      else  if( lvl & PERM_CHATCLOAK ) strcpy(buf,"ÎÞµÐÅ£Ê­");
      else if  ( lvl & PERM_BOARDS ) strcpy(buf,"¶þµ±¼ÒµÄ");
      else if ( lvl & PERM_DENYPOST ) strcpy( buf, "ÑÆ°Í" ); 
      else if ( ! (lvl & PERM_BASIC) ) strcpy( buf, "Ï¹×Ó" ); 
      else strcpy(buf,"¸«Í·°ïÖÚ");
    return 1;
}
/* Ê±¼ä×ª»»³É ÖÐÎÄ */
/*
...
char *direct ;
{
    ...
    sprintf(counterfile,"/home/system/bbs/Haohmaru/counter/%s.counter",currentuser.userid);
...
char quote_mode;
{
    ...
    if( *qfile != '\0' && (inf = fopen( qfile, "r" )) != NULL ) {  /* ´ò¿ª±»ÒýÓÃÎÄ¼þ*/
        ...
        if( op != 'N' ) {                 /* ÒýÓÃÄ£Ê½Îª N ±íÊ¾ ²»ÒýÓÃ */
            ...
            else fprintf( outf, "¡¾ ÔÚ %s µÄÀ´ÐÅÖÐÌáµ½: ¡¿\n", quser );
...
char *direct ;
{
    ...
    if (!strcmp(currboard, "syssecurity")
		...
		if( 0!=strcmp(currentuser->userid,"zixia") )
        		return DONOTHING ;
...
notepad()
{
    ...
    clear();
    move(0,0);
    prints("¿ªÊ¼ÄãµÄÒÅÑÔ°É£¡´ó¼ÒÕýÊÃÄ¿ÒÔ´ý....\n");
    ...
    if( (in = fopen( tmpname, "w" )) != NULL ) {
        ...
        while(1)
        {
            ...
            getdata(5,0,"ÊÇ·ñ°ÑÄãµÄ´ó×÷·ÅÈëÒÅÑÔ°å (Y)ÊÇµÄ (N)²»Òª (E)ÔÙ±à¼­ [Y]: ",note1, 3, DOECHO, NULL,YEA);
...
Goodbye()    /*ÀëÕ¾ Ñ¡µ¥*/
{
    ...
    prints("[[33m2[m] ·µ»Ø[32m*´ó»°Î÷ÓÎBBS*[m\n");
    if(strcmp(currentuser->userid,"guest")!=0){
        if( USE_NOTEPAD == 1)
            prints("[[33m3[m] Ð´Ð´*ÒÅÑÔ°æ*[m\n");
    }
