...
void output_ansi_html(char *buf, size_t buflen, buffered_output_t * output,char* attachlink)
{
				...
				case UBB_TYPE_ATTACH:
					if (!strcasecmp(UBBCode,"upload")){
						if ( (UBBArg1>0) && (UBBArg1<=attachmatched)) {
							...
							case ATTACH_IMG:
								snprintf(outbuf, 255, "<br>Í¼Æ¬:%s(%d ×Ö½Ú)<br><img src='%s&amp;ap=%d'></img><br />", attachFileName[UBBArg1-1], attachLen[UBBArg1-1], attachlink, attachPos[UBBArg1-1]);
								break;
							case ATTACH_FLASH:
	...
	for ( i = 0; i<attachmatched ; i++ ){
		if (!attachShowed[i]) { 
			...
			switch(attachType[UBBArg1-1]) {
			case ATTACH_IMG:
		 		snprintf(outbuf, 255, "<br>Í¼Æ¬:%s(%d ×Ö½Ú)<br><img src='%s&amp;ap=%d'></img><br />", attachFileName[i], attachLen[i], attachlink, attachPos[i]);
				break;
			case ATTACH_FLASH:
		                snprintf(outbuf, 255, "<br>Flash¶¯»­: " "<a href='%s&amp;ap=%d'>%s</a> (%d ×Ö½Ú)<br>" "<OBJECT><PARAM NAME='MOVIE' VALUE='%s&amp;ap=%d'>" "<EMBED SRC='%s&amp;ap=%d'></EMBED></OBJECT><br />", attachlink, attachPos[i], attachFileName[i], attachLen[i], attachlink, attachPos[i], attachlink, attachPos[i]);
				break;
			case ATTACH_OTHERS:
