...
void output_ansi_html(char *buf, size_t buflen, buffered_output_t * output,char* attachlink)
{
    ...
    for (i = 0; i < buflen; i++) {
		...
		if ( buf[i] == '[' )	{ //UBB¿ØÖÆ´úÂë¿ªÊ¼?
			...
			if (STATE_ISSET(ansi_state, STATE_UBB_START))	{
				...
				case UBB_TYPE_IMG:
					if (!strcasecmp(UBBCode,"img")){
						output->output("<br><img src=\"images/files/img.gif\">Í¼Æ¬£º<br><IMG SRC=\"", 56, output);
				...
				case UBB_TYPE_ATTACH:
					if (!strcasecmp(UBBCode,"upload")){
						if ( (UBBArg1>0) && (UBBArg1<=attachmatched)) {
							...
							case ATTACH_IMG:
								snprintf(outbuf, 511, "<br>Í¼Æ¬:%s(%d ×Ö½Ú)<br><img src='%s&amp;ap=%d'></img><br />", attachFileName[UBBArg1-1], attachLen[UBBArg1-1], attachlink, attachPos[UBBArg1-1]);
								break;
							case ATTACH_FLASH:
	...
	for ( i = 0; i<attachmatched ; i++ ){
		if (!attachShowed[i]) { 
			...
			switch(attachType[UBBArg1-1]) {
			case ATTACH_IMG:
		 		snprintf(outbuf, 511, "<br>Í¼Æ¬:%s(%d ×Ö½Ú)<br><img src='%s&amp;ap=%d'></img><br />", attachFileName[i], attachLen[i], attachlink, attachPos[i]);
				break;
			case ATTACH_FLASH:
